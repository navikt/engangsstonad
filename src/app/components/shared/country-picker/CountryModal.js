...
import bokmalContriesList from 'i18n-iso-countries/langs/nb.json';
import nynorskContriesList from 'i18n-iso-countries/langs/nn.json';
...
class CountryModal extends Component {
	constructor(props) {
		...
		const contriesLanguage =
			props.language === 'nb' ? bokmalContriesList : nynorskContriesList;
		...
		if (props.visit) {
			...
			this.state = {
				...
				submitButtonText: getMessage(
					intl,
					'medlemmskap.landvelger.leggTilLand'
				),
				land: '',
				startDato: '',
				sluttDato: ''
