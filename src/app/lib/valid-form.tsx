...
class ValidForm extends React.Component<Props, State> {
    ...
    onChange(_e: any, component: React.Component) {
        if (this.state.failedSubmit) {
            this.validateOne(component);
        }
    }

    onBlur(_e: any, component: React.Component) {
        if (this.state.failedSubmit) {
            this.validateOne(component);
        }
    }
    ...
    render() {
        ...
        if (this.state.failedSubmit && !this.state.valid && !noSummary) {
            summaryBox = (
                <FeilOppsummeringBoks title={summaryTitle} show={true} errors={this.mapResultsToErrorSummary()} />
            );
        }
